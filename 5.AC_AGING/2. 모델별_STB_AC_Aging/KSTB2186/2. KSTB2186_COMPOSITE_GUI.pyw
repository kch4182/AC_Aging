...
STB_NAME = "KSTB2186"             # 0. STB Model명
MOVE_IDIS = (535,260)             # 1. 색상 확인 좌표(IRIS)
MOVE_SMART = (290,260)            # 2. 색상 확인 좌표(SMART)  
AC_AGING_IDIS = (145, 187, 213)   # 3. Booting 완료 색상(IRIS)
AC_AGING_SMART = (6, 42, 253)  # 4. Booting 완료 색상(SMART)
APPLY_SCRIPT = "STB_AC_AGING_v06_COMPOSITE.py"
'''******************경로 수정 필요******************'''
...
root.geometry("1400x700")  # 창 크기 설정
...
tool_combobox['values'] = ("SMARTVIEWER", "IDIS")  # 옵션 설정
...
tk.Radiobutton(radio_frame, text="STB1 기준", variable=mode_var, value="stb1", command=update_coordinate_mode).pack()
tk.Radiobutton(radio_frame, text="개별 설정", variable=mode_var, value="individual", command=update_coordinate_mode).pack()

# STB 좌표 입력
coordinate_entries = {}

coord_frame = tk.Frame(root)
tk.Label(coord_frame, text="STB 좌표 입력").pack()
...
tk.Label(root, text="4. AC Setting").grid(row=4, column=0, padx=10, pady=5, sticky="w")
# 4. STB ON Time
tk.Label(root, text="On Check Time (초)").grid(row=5, column=0, padx=10, pady=5, sticky="w")
on_time_entry = tk.Entry(root)
on_time_entry.insert(0, "60")
on_time_entry.grid(row=5, column=1, padx=10, pady=5, sticky="w")

# 3. STB OFF Time
tk.Label(root, text="Off Check Time (초)").grid(row=6, column=0, padx=10, pady=5, sticky="w")
off_time_entry = tk.Entry(root)
off_time_entry.insert(0, "5")
off_time_entry.grid(row=6, column=1, padx=10, pady=5, sticky="w")

# 4. STB Check Time
tk.Label(root, text="Check Time (초)").grid(row=7, column=0, padx=10, pady=5, sticky="w")
check_time_entry = tk.Entry(root)
check_time_entry.insert(0, "50")
check_time_entry.grid(row=7, column=1, padx=10, pady=5, sticky="w")

# 5. 폴더 경로를 저장할 변수
path_label = tk.Label(root, text="5. 로그 저장 경로 선택")
path_label.grid(row=8, column=0, padx=10, pady=5, sticky="w")

folder_path = tk.StringVar()

choose_folder_button = tk.Button(root, text="경로 지정", command=choose_folder)
choose_folder_button.grid(row=8, column=1, padx=10, pady=5, sticky="w")

folder_label = tk.Label(root, textvariable=folder_path)
folder_label.grid(row=9, column=1, padx=10, pady=5, sticky="w")

# 6. 제출 버튼
submit_button = tk.Button(root, text="Start AC Check", command=lambda: threading.Thread(target=start_ac_check).start())
submit_button.grid(row=10, column=1, padx=10, pady=5, sticky="w")

# 7. 출력 결과 표시 Text 위젯
tk.Label(root, text="결과 출력").grid(row = 0, column =2, padx=10, pady=5, sticky="news")
output_text = scrolledtext.ScrolledText(root, height=30, width=95, font=("맑은 고딕", 10))
