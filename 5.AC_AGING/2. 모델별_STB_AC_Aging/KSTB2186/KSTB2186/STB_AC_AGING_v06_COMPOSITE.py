...
def ac_check(image, positions, off_check_time, STB_AC_LIST):
    global start_time, stb_rgb_counts, STB_AC_CHECK, STB_AC_CHECK_POSITIONS, CHANGE_CHECK_STB, stb_check_counts, count
            ...
            if start_time is not None and int((datetime.now() - start_time).total_seconds()) >= off_check_time:
                ...
                while True:
                    ...
                    if int((datetime.now() - start_time).total_seconds())>= (STB_AC_CHECK_TIME + STB_AC_OFF_TIME -0.5):
                        print("￣￣￣￣￣￣￣￣￣￣￣￣￣￣￣연산시작￣￣￣￣￣￣￣￣￣￣￣￣￣￣￣", flush=True)
                        for _ in range(10):
                            image_check = capture_screen()  # 캡처 화면 업데이트
                            stb_rgb_counts[error_check_stb]["last_print"] = error_check(image_check, STB_AC_POSITIONS, stb_log_status, first_print, stb_key, check_positions_x, check_positions_y)
                            first_print = False
                        start_time = None  # 연산 완료 후 초기화
                        # ON Time 충족할 때까지 while 문 계속 돌림
                        
                        while True :
                            if int((datetime.now() - DEAD_STB_COVER).total_seconds()) >= STB_AC_ON_TIME-1:
...
# Error Check 함수
def error_check(image, positions, stb_log_status, first_print, check_stb_key,check_positions_x,check_positions_y):
    global stb_rgb_counts, stb_check_counts
    for i, (x, y) in enumerate(positions):
        if x >= MAIN_WIDTH:
            x = x - MAIN_WIDTH  # 서브 모니터 좌표를 메인 모니터에 맞게 변환
        rgb = tuple(image[y, x])
        check_rgb = tuple(image[check_positions_y,check_positions_x])
        stb_key = f"STB {STB_AC_LIST[i]+1}"
        # 중복 IF문 진입 방지
        already_flag = stb_log_status[stb_key].get("last_log", None)
        
        # 0. Check Positions BLUE_RGB CHECK
        if np.linalg.norm(np.array(check_rgb) - np.array(BLUE_RGB)) < 20: # Check_RGB가 Blue 이면 
            stb_check_counts[check_stb_key]["screen_blue"] += 1 # count => 10번 찍히면
        else : 
            stb_check_counts[check_stb_key]["screen_blue"] = 0

        # print("ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ")
        # 1. Black Screen 체크
        if np.linalg.norm(np.array(rgb) - np.array(BLACK_RGB)) < 20:
            ...
            if stb_rgb_counts[stb_key]["black_count"] == 2 :
                ...
                # None 값도 아니고 이전 last_rgb 값과 비슷하지 않다면 값 초기화 
                    ...
                    if np.linalg.norm(np.array(rgb) - np.array(stb_rgb_counts[stb_key]["last_rgb"])) < 20 :
                        pass
        ...
        #####################################################################
        # 노이즈 발생 시 범위 수정 ----> 40에서 더 늘려도 됨
        # 3. Aging Complete 색상 체크 
        if np.linalg.norm(np.array(rgb) - np.array(AC_AGING)) <  40 :
