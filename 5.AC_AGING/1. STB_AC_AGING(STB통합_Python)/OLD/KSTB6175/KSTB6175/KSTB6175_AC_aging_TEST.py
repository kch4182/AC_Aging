...
'''
print("0:",sys.argv[0]) # 파이썬 파일 경로
print("1:",sys.argv[1]) # --stb_list
print("2:",sys.argv[2]) # STB_List
print("3:",sys.argv[3]) # -- on time
print("4:",sys.argv[4]) # 60
print("5:",sys.argv[5]) # -- off time
print("6:",sys.argv[6]) # 5
print("7:",sys.argv[7]) # --check_time
print("8:",sys.argv[8]) # 50
print("14:",sys.argv[14]) # AC_AGING_IRIS
print("16:",sys.argv[16]) # AC_AGING_SNART
print("18:",sys.argv[18])
print("20:",sys.argv[20])
'''
...
def log_event(stb_key, event, already_flag):
    ...
    if already_flag != compare_log :  # 이미 기록된 시간과 같으면 기록 안함
        ...
        log_file = os.path.join(log_path, f"_{START_DAY}.csv")
...
def ac_check(image, positions, off_check_time, STB_AC_LIST):
    ...
    for i, (x, y) in enumerate(positions):
        ...
        # BLUE_RGB_IRIS 상태가 일정 시간 동안 유지되면 start_time 기록
        if np.linalg.norm(np.array(rgb) - np.array(BLUE_RGB)) < 15 :
            if start_time is None:
                start_time = time.time()  # BLUE_RGB_IRIS 상태가 지 속되면 start_time 기록

            # 화면이 BLUE_RGB_IRIS 상태가 유지된 시간이 off_check_time 이상이면 비교 시작
            if start_time is not None and time.time() - start_time >= off_check_time:
                print(f"{stb_key}: IRIS 상태가 {off_check_time}초 동안 유지됨")
                while True:
                    if time.time() - start_time >= (STB_AC_CHECK_TIME + STB_AC_OFF_TIME):
