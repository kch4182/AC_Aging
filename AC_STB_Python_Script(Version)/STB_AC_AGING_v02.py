    ...
    for i, (x, y) in enumerate(positions):
        ...
        if np.linalg.norm(np.array(rgb) - np.array(BLUE_RGB)) < 20:
            ...
            if start_time is not None and int((datetime.now() - start_time).total_seconds()) >= off_check_time:
                ...
                print(f"[{datetime.now()}]:{stb_key}: STB OFF가 {off_check_time}초 동안 유지됨")
                time.sleep(0.5)
                while True:
                    # 연산 시작 조건 확인
...
print("***Main 화면 창에 IRIS or SMARTVIEWER 전체화면 후 최상단 배치***")
time.sleep(5)
