...
BLUE_RGB_IDIS = (10,4,240)  # IDIS 화면 Blue (2, 231, 5)print("여기 확인 필요")
BLUE_RGB_SMART = (8, 8, 252)  # SmartViewer화면 Blue
# Composite 화면 끊김 변수
SMART_WISENET = (31,31,31)
STB_POSITIONS_IDIS  = [
    (0, 0), (640, 0), (1280, 0),
...
def ac_check(image, positions, off_check_time, STB_AC_LIST):
    global start_time, stb_rgb_counts, STB_AC_CHECK, STB_AC_CHECK_POSITIONS, CHANGE_CHECK_STB
    for i, (x, y) in enumerate(positions):
         # 만약 서브 모니터에 해당하는 좌표라면
        if x >= MAIN_WIDTH:
            x = x - MAIN_WIDTH  # 서브 모니터 좌표를 메인 모니터에 맞게 변환
        print("xtype:", type(x), type(y))
        rgb = tuple(image[y, x])
        stb_key = f"STB {STB_AC_LIST[CHANGE_CHECK_STB]+1}"
        ...
        if np.linalg.norm(np.array(rgb) - np.array(BLUE_RGB)) < 20 :
            ...
            stb_rgb_counts[stb_key]["screen_blue"] += 1 # 0.3초마다 하나씩 counts 1분이면 -> 200    
            
            # OFF과 Check 연산시 BLUE 계속 유지되면 Check STB 기준 넘어감
            if stb_rgb_counts[stb_key]["screen_blue"] > 70 :
                already_flag= 0
                ...
                stb_rgb_counts[stb_key]["screen_blue"] = 0 # 초기화
                break
                ...
                while True:
                    ...
                    if int((datetime.now() - start_time).total_seconds())>= (STB_AC_CHECK_TIME + STB_AC_OFF_TIME -0.5):
                        ...
                        for _ in range(10):
                            ...
                            stb_rgb_counts[stb_key]["last_print"] = error_check(image_check, STB_AC_POSITIONS, stb_log_status, first_print)
                            first_print = False
...
# Error Check 함수
def error_check(image, positions, stb_log_status, first_print):
