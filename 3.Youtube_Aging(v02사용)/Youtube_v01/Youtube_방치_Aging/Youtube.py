...
# 상태 구분
stb_rgb_counts = {
    f"STB {i+1}": {"count": 0, "last_rgb": None, "black_count" :0, } for i in range(len(STB_POSITIONS))
}
...
def error_check(image, positions, stb_log_status):
    ...
    for i, (x, y) in enumerate(positions):
        ...
        if np.linalg.norm(np.array(rgb) - np.array(BLACK_RGB)) < 12:
            stb_rgb_counts[stb_key]["black_count"] += 1 # count => 10번 찍히면 
            if  stb_rgb_counts[stb_key]["black_count"] == 3 :
        ...
        # Screen 끊김 체크
            ...
            # print("2. Screen : ", np.linalg.norm(np.array(rgb) - np.array(BLUE_RGB)))
        # Youtube 색상 체크

        elif np.linalg.norm(np.array(rgb) - np.array(YOUTUBE_24H)) <  15 :
            # print("3. Complete : ", np.linalg.norm(np.array(rgb) - np.array(YOUTUBE_24H)))
            pass

        # 24h 영상(Pink 색상) -> 기준으로 비슷하지 않다면 Log 저장
        else :
