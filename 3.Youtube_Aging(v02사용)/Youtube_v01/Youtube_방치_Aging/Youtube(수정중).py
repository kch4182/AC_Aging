...
'''*********************************STB Model 마다 변경해야 할 거*********************************'''
compare_YOUTUBE = (255,172,202)
compare_YOUTUBE_2 = (248,163,193)
'''*********************************IRIS or SmartViewer Tool 적용*********************************'''
BLUE_RGB_IRIS = (2, 231, 5)  # IRIS 화면 Blue
BLUE_RGB_SMART = (8, 8, 252)  # SmartViewer화면 Blue
YOUTUBE_24H_IRIS = (220,151,171)    # 1. Youtube 색상(IRIS)
YOUTUBE_24H_SMART = (253,167,196)   # 2. Youtube 색상(SMART)
...
# 좌표 확인용 함수
def show_stb_positions(image, positions):
...
### 상태 구분
stb_rgb_counts = {
    f"STB {i+1}": {"screen_black": 0,"screen_blue":0,"screen_stop":0, "last_rgb": None, "black_count" :0, } for i in range(len(STB_POSITIONS))
}
### 
max_com1 = 0
max_com2 = 0
...
def error_check(image, positions, stb_log_status):
    global black_count # State 구분 count
    global max_com1, max_com2, max_youtube
    for i, (x, y) in enumerate(positions):
        ...
        time.sleep(1)
        '''ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ최대값 지울거ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
        
        if max_com1 < np.linalg.norm(np.array(rgb) - np.array(compare_YOUTUBE)) :
            max_com1 = np.linalg.norm(np.array(rgb) - np.array(compare_YOUTUBE))
        
        if max_com2 < np.linalg.norm(np.array(rgb) - np.array(compare_YOUTUBE_2)) :
            max_com2 = np.linalg.norm(np.array(rgb) - np.array(compare_YOUTUBE_2))
        
        if max_youtube < np.linalg.norm(np.array(rgb) - np.array(YOUTUBE_24H)) :
            max_youtube = np.linalg.norm(np.array(rgb) - np.array(YOUTUBE_24H))
        
        print(max_com1)
        print(max_com2)
        print(max_youtube)
        ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ지울거ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ'''

        # 1. Black Screen 체크
        if np.linalg.norm(np.array(rgb) - np.array(BLACK_RGB)) < 12:
            ...
            elif stb_rgb_counts[stb_key]["screen_black"] < {(CAPTURE_INTERVAL * 200)* 4} : # 60초 * 4 => 4분 이상 지속되면 로그 찍지 않음
                # black이 4분 정도 유지 된다면 로그 더 이상 찍지 않음
                if np.linalg.norm(np.array(rgb) - np.array(stb_rgb_counts[stb_key]["last_rgb"])) < 12 :
                    pass
                # 4분 이상 지속되다 색상이 변한다면 rgb 값, count 초기화
                elif stb_rgb_counts[stb_key]["last_rgb"] is None : 
                    stb_rgb_counts[stb_key]["last_rgb"] = np.array(rgb)
                    stb_rgb_counts[stb_key]["screen_black"] = 0
             

        # 2. Screen 끊김 체크
        elif np.linalg.norm(np.array(rgb) - np.array(BLUE_RGB)) < 15 :
            stb_rgb_counts[stb_key]["screen_blue"] += 1 # count => 10번 찍히면 
            if  stb_rgb_counts[stb_key]["screen_blue"] < {(CAPTURE_INTERVAL * 200)* 4} :
                if np.linalg.norm(np.array(rgb) - np.array(stb_rgb_counts[stb_key]["last_rgb"])) < 12 :
                    pass
                elif stb_rgb_counts[stb_key]["last_rgb"] is None : 
                    stb_rgb_counts[stb_key]["last_rgb"] = np.array(rgb)
        ...
        # 3. Youtube 색상 체크 
        elif np.linalg.norm(np.array(rgb) - np.array(YOUTUBE_24H)) <  15 :
            pass
        ...
        # 4. 24h 영상(Pink 색상) -> 기준으로 비슷하지 않다면 Log 저장
            ...
            if  stb_rgb_counts[stb_key]["screen_stop"] < {(CAPTURE_INTERVAL * 200)* 4} :
                if np.linalg.norm(np.array(rgb) - np.array(stb_rgb_counts[stb_key]["last_rgb"])) < 12 :
                    pass
                elif stb_rgb_counts[stb_key]["last_rgb"] is None : 
                    stb_rgb_counts[stb_key]["last_rgb"] = np.array(rgb)
...
print("***7초 후 Test 좌표 캡처 시작***IRIS or SMARTVIEWER 창 전체화면 후 최상단 배치***")
time.sleep(5)
screenshot = capture_screen()
show_stb_positions(screenshot, STB_AGING_POSITIONS)
time.sleep(10)
print("10초 후 Youtube Aging Detecting Start ")
main()
